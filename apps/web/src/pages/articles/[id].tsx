...
const ArticlePage = () => {
  ...
  const backTitle = trpc.useQuery([
    "article.getBackTitle",
    { postId, questionId },
  ]);
  ...
  const articleComentVotes = trpc.useQuery([
    "article.getArticleCommentVotes",
    { articleId },
  ]);
  ...
  const renderTopSupporting = () => {
    const article = articleData.evidencePosts.find(
      (e) => e.evidenceType == "FOR"
    );
    if (!article) {
      return null;
    }
    ...
    <Page title={articleData.title}>
        <div className="grid grid-cols-5 gap-4 max-w-screen-xl mx-auto">
          <div className="flex flex-col col-span-3 space-y-4">
            {backTitle.data && (
              <BackTitle
                url={(questionId && `/questions/${questionId}`) || undefined}
                        ...
                        .map((evidencePostData) => (
                          <EvidenceArticle
                            ...
                            evidenceArticleCount={
                              evidencePostData.evidencePost._count.parentPosts
                            }
                          />
                        ))}
                    </div>
