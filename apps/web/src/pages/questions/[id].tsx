...
const QuestionPage = () => {
  ...
  const scheller = schellings.filter(
    (x) => x.state === "PENDING" && x.userId === userID.data
  )[0];
 ...
 const consensusCompleted = schellings.filter(x => x.state === 'SUBMITTED')
  ...
  useEffect(() => {
    ...
    setQuestionFour(
      scheller?.personalAnswer === null ? true : !!scheller?.personalAnswer
    );
  }, [scheller]);

  const activeButtonCss =
    "bg-yellow-600 rounded-md p-4 font-bold text-white grow basis-1/2";
  const inactiveButtonCss =
    "border-yellow-600 border text-yellow-600 rounded-md py-2 font-bold grow basis-1/2";

  if (!router.isReady || question.isLoading || !questionData) {
    return <LoadingArticlePage />;
  }

  const forPostsCount = questionData.posts.filter(
    (p) => p.evidenceType === "FOR"
  ).length;
  const againstPostsCount = questionData.posts.filter(
    (p) => p.evidenceType === "AGAINST"
  ).length;

  const forPercent =
    questionData.posts.length > 0
  ...
  const renderStep = () => {
      ...
      return (
        <>
          <p className="text-sm text-justify">
            Based on all the evidence presented, do you think most people would
            say yes or no to the question above?
          </p>
          ...
          <button
            className={`flex justify-end font-semibold text-stone-500 ${
              promptNext ? "animate-bounce" : ""
            }`}
            disabled={!promptNext}
            onClick={() => {
              if (promptNext) {
                ...
                .map((_, i) => {
                  return (
                    <>
                      ...
                      {i !== 4 && (
                        <div className="grow bg-yellow-600 h-1"></div>
                      )}
                    </>
                  );
                })}
    ...
    if (step === 2) {
      return (
        <>
          <p className="text-sm text-justify">
            Which post was most influential in helping you make your decision?
          </p>
          <select
            onClick={() => {
              setPromptNext(true);
    ...
    if (step === 3) {
      return (
        <>
          <p className="text-sm text-justify">
            Based on all the evidence presented, would you personally vote
            &ldquo;yes&rdquo; or &ldquo;no&rdquo; to the question on the left?
            This is for internal use only and has no bearing on your reward
            eligibility.
          </p>
          <div className="flex space-x-2">
            <button
    ...
    if (step === 4) {
      return (
        <>
          <p className="text-sm text-justify">
            Thank you for participating in the truth consensus algorithm
          </p>
          <div className="flex space-x-2">
            <button
        ...
        <div className="grid grid-cols-3 gap-4 max-w-screen-xl mx-auto">
          {!votingMode && (
            <div className="p-2 flex flex-col space-y-2">
              ...
              <div className="bg-zinc-200 rounded-md p-4 flex flex-col">
                ...
                <div className="p-4 bg-slate-300 rounded-md flex flex-col">
                  ...
                  <div className="bg-gray-200 rounded-xl">
                    <div
                      className={`bg-green-600 rounded-xl h-2`}
                ...
                <div className="p-4 rounded-md flex flex-col">
                  ...
                  <div className="bg-gray-200 rounded-xl">
                    <div
                      className={`bg-red-500 rounded-xl h-2`}
              ...
              <Link
                href={`/articles/newWithQuestion/?questionId=${questionId}`}
                legacyBehavior>
                <button className="bg-yellow-600 rounded-md p-4 font-bold text-white">
                  POST YOUR VIEW
                </button>
              </Link>
          ...
          <div className="flex flex-col space-y-2 col-span-2">
            ...
            {isAdmin.data && !finalizeConsensus.isSuccess && questionState === 'ACTIVE' && (
              <div className="bg-white rounded-md p-2 px-4 flex justify-around items-center">
                ...
                <button className="bg-green-600 rounded-md p-3 text-white font-bold text-sm" disabled={finalizeConsensus.isLoading} onClick={() => {
    ...
    <div className="bg-white rounded-md p-2 px-4 flex flex-col space-y-2 shadow hover:shadow-lg transition">
      <div className="flex space-x-8 text-yellow-800 items-center">
        <div className="flex space-x-2 items-center">
          <div className={`${
                evidenceType === "FOR" ? "bg-green-600" : "bg-red-600"
          ...
          <div className="text-sm flex">
            ...
            <div
              className={`${
                evidenceType === "FOR" ? "text-green-600" : "text-red-600"
              } font-bold`}
            >
              {evidenceType}
            </div>
