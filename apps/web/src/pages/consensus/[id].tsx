import ConsensusScreen from '@/components/mobile/screens/ConsensusScreen'
...
const QuestionPage = () => {
	...
	const question = trpc.useQuery(['question.getQuestionWithConsensus', { id: questionId }])
	...
	return (
		<>
			...
			{width > 992 ? (
								...
								<div className='flex flex-col shadow px-8 py-4 rounded-lg space-y-4'>
									<div className='text-stone-600 flex justify-between items-center space-x-2 text-sm'>
										<div className='flex items-center space-x-2'>
											...
											<div>
												Posted by {questionData.author?.name} {timeToReadibleAgo(questionData.createdAt)}
											</div>
										</div>
									...
									<div className='whitespace-pre-line'>
										<span
											className={`italic pr-1 font-semibold ${
												questionData.Consensus?.answer === 'FOR' ? 'text-green-600' : 'text-red-800'
											}`}
										>
											{questionData.Consensus?.answer === 'FOR' ? 'YES' : 'NO'} ({questionData.Consensus?.confidence}{' '}
											level of confidence)
										</span>
													...
													{questionData.posts
														...
														.map((evidencePostData) => (
															<EvidenceArticle
																...
																evidenceArticleCount={evidencePostData.evidencePost._count.evidencePosts}
															/>
														))}
												</div>
											</div>
				...
				<ConsensusScreen consensusData={questionData} evidenceView={evidenceView} setEvidenceView={setEvidenceView} />
			)}
		</>
	)
}
