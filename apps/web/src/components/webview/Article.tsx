			...
			{width > 992 ? (
				<div className='rounded-md p-2 px-4 flex flex-col space-y-2 shadow bg-white '>
					...
					<div className='mx-4 flex space-x-2'>
						{avatar ? (
							<Image alt='commenter avatar' width={24} height={24} className='rounded-full w-6 h-6' src={avatar} />
						) : (
					...
					<div className='mx-4 overflow-clip'>
						...
						{url &&
							url.split(',').map((u) => (
								<Link href={u} key={u} target='_blank' className='text-gold hover:text-yellow-400 flex items-center'>
				...
				<div className='bg-white  p-2'>
					<CardHeader author={{ name: postedByName, id }} createdAt={postedAt} />
					...
					<Link href={`/articles/${articleId}/evidences`}>
						<div className='flex justify-center'>
							<button className='p-2 text-gold flex items-center rounded-lg border border-gold'>
								<Image width={40} height={40} src={'/logo.svg'} alt='delvit logo' />
								<span className='mx-2'>{evidencePosts?.length} Evidence blocks</span>
							</button>
						</div>
					</Link>
