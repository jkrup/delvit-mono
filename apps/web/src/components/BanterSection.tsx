...
const BanterSection: React.FC<BanterSectionProps> = ({ articleId, questionId }) => {
	...
	return (
		<div className='flex flex-col text-black rounded-md space-y-4 text-sm'>
			<form ref={commentFormRef} className='group' onSubmit={handleSubmitComment}>
				<div className='flex space-x-3'>
					<textarea
						className='bg-stone-100 rounded-md p-1 px-3 w-full resize-none group-focus-within:h-16 transition-all'
						name='comment'
						required
						placeholder='Add banter'
					/>
					<button className='group-focus-within:block hidden bg-amber-400 rounded-md px-3 items-center'>Comment</button>
					{/* <div className="so text-xs italic text-gray-400 text-right">Hold Shift+Enter to add a new line. Enter to submit.</div> */}
				</div>
			</form>

			<div className='flex flex-col'>
				{orderedComments?.map(({ id, depth = 0, votes, body, author, createdAt, hasVoted }) => {
