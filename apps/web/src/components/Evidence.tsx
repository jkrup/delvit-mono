...
const Evidence: React.FC<EvidenceProps> = ({ id, title, type, children, evidenceLink, stance, lost, commentsLink }) => {
		...
		<div className={`flex flex-col rounded-md ${lost ? 'opacity-50' : ''}`}>
			...
			>
				...
				<div className='h-42 mx-1 flex flex-col flex-grow'>
					...
					<div className='text-sm line-clamp-4'>{bodyStringToMarkup(children as string)}</div>
				</div>
				<div className='flex flex-col justify-between text-gray-400 mx-1 text-sm'>
					{evidenceLink && (
						<Link href={`${evidenceLink}`} className='text-blue-500 line-clamp-2' target='_blank'>
							{evidenceLink}
						</Link>
					)}
					<a>
